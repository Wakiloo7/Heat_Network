...
# phi_ng = {(1, 1), (2, 2), (3, 3)}  # Add the constraint set for the gas nodes

# Define the parameters
C_mn = {(1, 2): 1.0, (2, 3): 1.2} #Cost of gas flow between nodes m and n
P_n_min = {1: 10, 2: 15, 3: 20} #Minimum gas pressure at node n
P_n_max = {1: 100, 2: 90, 3: 80} #Maximum gas pressure at node n
G_n_min = {1: 5, 2: 10, 3: 15} #Minimum gas flow at node n
G_n_max = {1: 200, 2: 100, 3: 150} #Maximum gas flow at node n
...
# # Define the parameters
# C_mn = {(1, 2): 1.0, (2, 3): 1.2}
# P_n_min = {1: 5, 2: 10, 3: 15}
# P_n_max = {1: 150, 2: 140, 3: 130}
# G_n_min = {1: 1, 2: 1, 3: 1}
# G_n_max = {1: 250, 2: 200, 3: 150}
# gf_mn_max = {(1, 2): 1000, (2, 3): 800}
...
model.P_kn = pyo.Var(phi_k, phi_n, within=pyo.NonNegativeReals, initialize=lambda model, k, n: P_n_min[n])
model.G_kn = pyo.Var(phi_k, phi_n, within=pyo.NonNegativeReals, initialize=lambda model, k, n: G_n_min[n])

# Add slack variables for the gas volume balance constraint
model.Slack_kn = pyo.Var(phi_k, phi_n, within=pyo.NonNegativeReals)
...
def gas_volume_balance_rule(model, k, n):
    inflow = sum(model.gf_kmn[k, m, nn] for m, nn in phi_gmn if nn == n)
    outflow = sum(model.gf_kmn[k, mm, n] for mm, n in phi_gmn if mm == n)
    return model.G_kn[k, n] == inflow - outflow + model.Slack_kn[k, n]

model.GasVolumeBalance = pyo.Constraint(phi_k, phi_n, rule=gas_volume_balance_rule)
...
def mccormick1(model, k, m, n):
    return model.Gamma_kmn[k, m, n] >= (1 / C_mn[m, n]) ** 2 * model.gf_kmn[k, m, n] ** 2
...
#Add constraints for minimum and maximum gas values
...
def objective_function(model):
    return sum(C_mn[m, n] * model.gf_kmn[k, m, n] for k in phi_k for m, n in phi_gmn)
